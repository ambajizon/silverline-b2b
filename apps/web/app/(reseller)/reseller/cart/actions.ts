...
export async function placeOrder(payload: CheckoutPayload) {
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      const { data: code, error: rpcError } = await sb.rpc('generate_order_code')
      if (code && !rpcError) {
        orderCode = code as string
        break
      }
    } catch (e) {
      console.log('RPC generate_order_code not available, using fallback')
    }
    
    // Fallback: Generate code with timestamp + counter
